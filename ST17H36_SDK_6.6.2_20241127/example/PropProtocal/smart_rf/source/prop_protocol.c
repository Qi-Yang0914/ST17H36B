...
#define DEFAULT_SYNCWORD                        (0x8e89bed6)//(0xed31d942)//(0x8e89bed6)
...
#define CHN_MAP_BIT_MSK_37                  (0x01)
#define CHN_MAP_BIT_MSK_38                  (0x02)
#define CHN_MAP_BIT_MSK_39                  (0x04)
#define CHN_MAP_BIT_MSK_ALL                 (0x07)
...
/*********************************************************************
    LOCAL FUNCTIONS
*/
static uint8_t phy_rx_data_check(uint8_t evt)
{
    pppMsgEvent_t *msg;
    uint8_t dlen = phyBufRx[1]+2; //inculde 2 byte header
    uint8_t* pBuf = osal_mem_alloc(dlen);
    if(pBuf==0)
    {
        LOG("osal_heap_full !\n");
        return 0;
    }
    osal_memcpy(pBuf,&(phyBufRx[0]),dlen);
    osal_memset(&(phyBufRx[0]),0,8);

// allocated space for OSAL message
    msg = (pppMsgEvent_t *)osal_msg_allocate( sizeof ( pppMsgEvent_t ) );
    ...
    if ( msg )
    {
      ...
      msg->rssi       = phyRssi;
      msg->foff       = phyFoff;
      msg->rfChn      = s_phy.rfChn;
      // send message
      (void)osal_msg_send( PPP_TaskID, (uint8 *)msg );
...
void phy_rx_data_process(pppMsgEvent_t * pMsg)
{
    uint8_t pduLen=pMsg->len;
    
    if(osal_memcmp(&(pMsg->pData[2]),peer_addr,6))
    {
        PRINT("------------------------- [%d]\n",pMsg->hdr.event);
        PRINT("[PHY RX] [-%03ddbm %4dKHz %02d CH] ",pMsg->rssi,pMsg->foff-512,pMsg->rfChn);

        for(uint8_t i=0; i<pduLen+2; i++)
            PRINT("%02x ",pMsg->pData[i]);
...
void PLUSPHY_IRQHandler(void)
{
            ...
            if(osal_memcmp(phyBufRx+2,peer_addr,6))    //20221020 ZONG: move addr check here, and check remote addr instead of local addr
            {
                if(DEF_PPP_AUTOACK_SUPPORT == 1)       //SRX:RX->TXACK
                {
                    if(!(PPP_GET_ACK_BIT(phyBufRx[0])) && (PPP_GET_NEEDACK_BIT(phyBufRx[0])))
                    {
                        ...
                        {
                            // report rx data
                            phy_rx_data_check(PPP_MSG_DATA_EVENT);
                        }
           ...
           )
    {
        ...
        if(irq_status & LIRQ_COK)
        {
            ...
            {
                s_txOkFlag=1;
                phy_rx_data_check(PPP_MSG_ACK_EVENT);
            }
            else
            {
                //wrong device, ignore
...
// post ISR process
    ...
    if(nextMode==PPP_RX_DONE_EVT)
        process_rx_done_evt();
    else if(nextMode==PPP_TX_DONE_EVT)
        process_tx_done_evt();
}
...
void PropProtocol_Init(uint8 task_id)
{
    ...
    LOG("[PHY] init done %d rfchn%d SW[%8x] CRC[%d %8x] WT[%2x] %d AUTOACK %d\n"\
        ,s_phy.Status,s_phy.rfChn,s_pktCfg.syncWord,s_pktCfg.crcFmt, s_pktCfg.crcSeed,s_pktCfg.wtSeed,s_pubAddr[0],DEF_PPP_AUTOACK_SUPPORT);
...
static void PPP_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
    switch ( pMsg->event )
    {  
        case PPP_MSG_DATA_EVENT:
        case PPP_MSG_ACK_EVENT:
            phy_rx_data_process((pppMsgEvent_t * )pMsg);
        break;
...
uint16 PPP_ProcessEvent(uint8 task_id, uint16 events)
{
    ...
    if ( events & SYS_EVENT_MSG )
    {
        ...
        if ( (pMsg = osal_msg_receive( PPP_TaskID )) != NULL )
        {
            PPP_ProcessOSALMsg( (osal_event_hdr_t *)pMsg );
