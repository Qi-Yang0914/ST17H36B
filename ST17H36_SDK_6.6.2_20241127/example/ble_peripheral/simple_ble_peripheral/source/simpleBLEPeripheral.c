...
void simpleBLEPeripheral_processAuthCompleteMsg(uint8 opcode, uint32 *pMsg)
{
	gapAuthCompleteEvent_t *msg = ( gapAuthCompleteEvent_t *)pMsg;
    LOG("sbp Auth opcode %d,%d\n",opcode,msg->hdr.status);
    osal_start_timerEx(application_TaskID,SBP_PERIODIC_EVT, 200);
    osal_start_timerEx(application_TaskID,SBP_SYSINFO_EVT, 20);
}
...
void sbp_GapStateCB( gaprole_States_t newState )
{
	...
	{
		HCI_LE_SetAdvParamCmd( 160, 160, 0,0,0,NULL,0x07,0);
