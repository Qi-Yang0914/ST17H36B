...
#include "flash.h"
#define TOGGLE_GPIO P0
uint8_t id_buf[4095];
void fs_timing_test(void)
{
    static uint8_t testCase = 6;
    uint8_t data_wr[4]= {0x12,0x34,0x56,0x78};
    uint16_t i,file_len;
    int ret;
    uint32_t garbage_size,garbage_num;
    gpio_write(TOGGLE_GPIO,1);
    WaitMs(1);

    switch(testCase)
    {
    case 0:
        gpio_write(TOGGLE_GPIO,0);
        hal_flash_erase_sector(0x11050000);
        gpio_write(TOGGLE_GPIO,1);
        WaitMs(1);
        break;
    ...
    case 1:
        gpio_write(TOGGLE_GPIO,0);
        hal_flash_write(0x11050000,data_wr,4);
        gpio_write(TOGGLE_GPIO,1);
        WaitMs(1);
        break;
    ...
    case 2:
        gpio_write(TOGGLE_GPIO,0);
        osal_memcpy((uint8_t*)id_buf,(uint8_t*)0x11050000,4);
        gpio_write(TOGGLE_GPIO,1);
        WaitMs(1);
        break;
    ...
    case 3:
        gpio_write(TOGGLE_GPIO,0);
        ret = hal_fs_format(0x11050000,3);
        gpio_write(TOGGLE_GPIO,1);
        WaitMs(1);
