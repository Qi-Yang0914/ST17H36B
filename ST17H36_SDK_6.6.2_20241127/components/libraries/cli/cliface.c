...
#endif
void CLI_uart_process_data(comm_evt_t* pev)//uint8_t* buf, uint32_t len)
{
    cli_ctx_t* p_ctx = & g_cli_ctx;
    uint32_t i;
...
void __ATTR_FUNC_RAM__(CLI_UART0_IRQHandler)(void)
{
...
)
{
    ...
    ret = uart_init(baudrate, tx, rx, (void *)CLI_UART0_IRQHandler);
    JUMP_FUNCTION_SET(V11_IRQ_HANDLER, (uint32_t)&CLI_UART0_IRQHandler);
    uart0_fcr_fifo_control_register_set(0x81); // rx 1/2 trigger, enable fifo
    return ret;//uart init
}
...
int CLI_process_line_manual(uint8_t* buffer,uint32_t buffer_len)
{
    ...
    */
    ...
    // {
    //     AT_LOG("%d: ",j);
    //     for (int i = 0; i < CLI_strlen(argv[j]); i++)
    //     {
    //         AT_LOG("%c", argv[j][i]);
    //     }
    //     AT_LOG("\n");
    // }
    // AT_LOG("\n");
    if (0 == CLI_STR_COMPARE(buffer, g_cli_cmd_list[0].cmd))
    {
        g_cli_cmd_list[0].cmd_hdlr(argc, argv);
        matchflag = 1;
    }
    for (index = 1; index < g_cli_cmd_len; index++)
    {
// gpio_write(P14, 1);
// gpio_write(P14, 0);
// AT_LOG("--found at list index = %d --\r\n",index);
    ...
    if (matchflag == 2 || matchflag == 0)
    {
        AT_LOG("not match");
        AT_LOG("+ERROR\r\n");
        for (int i = 3; i < buffer_len; i++)
        {
            // AT_LOG("%c", buffer[i]);
            if (buffer[i] == '\0')
            {
