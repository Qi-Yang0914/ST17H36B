...
#define CLIT_output(...) {uart_send_buff(CLIT_preamble(NULL),4); CLI_OUT(__VA_ARGS__);}
#define CLIT_outputs(...) {uart_send_buff(CLIT_preamble(NULL),4); CLI_OUTc(__VA_ARGS__);}
#define CLIT_outputc(...) CLI_OUTc(__VA_ARGS__)
#define CLIT_outpute(...) CLI_OUT(__VA_ARGS__)

#define CLI_UART_INIT(prompt) CLI_uart_init(P4, P5, 115200, prompt)
