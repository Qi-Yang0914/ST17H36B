...
#define LL_SET_BITS(p,f,l,m)  p=(f<<l) | (p & (~m))
#define LL_GET_BITS(p,l,m)     (p&m)>>l
...
#define LL_ADV_CHAN_37                                 37
#define LL_ADV_CHAN_38                                 38
#define LL_ADV_CHAN_39                                 39
#define LL_ADV_CHAN_FIRST               				LL_ADV_CHAN_37
#define LL_ADV_CHAN_LAST                				LL_ADV_CHAN_39
...
//#define LL_EVT_RESET_SYSTEM_SOFT                 0x040
...
#define LL_MAX_CTRL_PROC_TIMEOUT                 32000 // 40s
...
#define LL_IMMEDIATE_HW_TRIG_CNT				  0xFFFFFFFF
...
typedef struct
{
  uint8_t     AddrType;                      // peer device address type of public or random
  uint8_t     Addr[ LL_DEVICE_ADDR_LEN ];                     // peer device address
} llAddrInfo_t;
...
typedef struct 
{
    uint32  		advInterval;                       // the advertiser interval, based on advIntMin and advIntMax
    /// Advertising type
    ...
    uint8_t       	advDataLen;                    // advertiser data length
  	uint8		  	duration;						// advertising data tx'ed duration time in 1Mbps
...
typedef struct
{
  ...
  uint8  SK[ LL_ENC_SK_LEN ] __attribute__((aligned(4)));                        // session key derived from LTK and SKD
...
//    uint16_t         lastSlaveLatency;               // last slave latency value used
//    uint16_t         slaveLatencyValue;              // current slave latency value (when enabled)
   ...
   uint8_t          lastCurrentChan;                // the channel for the last currentChan for disable slavelatency usage
    ...
    uint8_t          txDataQLen;                     // length of txdatatQ
...
typedef struct llScheduleInfo_t
{
	uint8   	  	busy;
	uint8			priority;
	llschRole_t     Role;
	uint8			index;
//	uint32        	task_period;     // Estimate how long the task will take       
//	uint32        	task_duration;            

	// trig_cnt : ll schedule timer counter 
	uint32        	trig_counter;
	
	struct llScheduleInfo_t	*next;
}  llScheduleInfo_t;
...
// Function prototype to process LLirq
typedef uint8 (*llProcessLLIRQ_t)( uint32_t      	irq_status);

extern void ll_schedule_update0(uint8 immSch);
extern uint8 ll_schedule_trig(llScheduleInfo_t* node);
extern llStatus_t llSetupPhyplysAdv( void );

typedef void (*llAESGetDataOut_t)( uint8 *ciphertext );
