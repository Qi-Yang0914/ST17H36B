...
#define LL_SCH_BASE                   (BB_HW_BASE+0x200)
...
#define LL_SCH_IRQ_ADDR                  (0xD8)
#define LL_SCH_TRIG_ADDR                 (0xDC)
#define LL_SCH_IRQ_CLR_ADDR				 (0xE8)

#define LL_SCH_CTL_POLLING               (0)
#define LL_SCH_CTL_AUTO                  (1)
#define LL_SCH_CTL_STOP                  (0xFF)
#define LL_TIFS_US						 (150)

#define LL_MAX_SCH_CNT					 (0x00FFFFFF)
#define LL_HALF_MAX_SCH_CNT				 (0x00800000)
#define LL_UNUSED_SCH_MSB				 (0xFF000000)

#define LL_SCH_TIME_DELTA(x,y)         ( (x>=y) ? (x)-(y) : LL_MAX_SCH_CNT-(y)+(x) )

typedef uint8 (*llProcessSCHIRQ_t)(void);
...
void ll_hw_set_rfphyFreqOff(uint8 rfChnIdx);

void ll_sch_clk_init(uint8_t div);
void ll_sch_cnt_config(uint32_t loadCnt);
void ll_sch_set_target(uint8_t addr,uint8_t mode,uint32_t target);
uint32_t ll_sch_get_current_time(void);
uint32_t ll_sch_get_mode_done_time(void);
uint8 ll_sch_get_irq_en_status(uint8_t addr);
uint8_t ll_sch_get_status(uint8_t addr);
uint8_t ll_sch_get_isruning(uint8_t addr);
//void calculate_whiten_seed(void);
uint32 ll_hw_get_rx_timeout_1st(void );
