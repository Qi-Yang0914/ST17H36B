...
// Characteristic 1 UUID: 0xFED5
CONST uint8 ppsp_serv_char_fed5_uuid[ATT_BT_UUID_SIZE] =
{
    LO_UINT16(PPSP_SERV_CFGS_CHAR_FED5_UUID), HI_UINT16(PPSP_SERV_CFGS_CHAR_FED5_UUID)
};

// Characteristic 2 UUID: 0xFED7
CONST uint8 ppsp_serv_char_fed7_uuid[ATT_BT_UUID_SIZE] =
{
    LO_UINT16(PPSP_SERV_CFGS_CHAR_FED7_UUID), HI_UINT16(PPSP_SERV_CFGS_CHAR_FED7_UUID)
};

// Characteristic 3 UUID: 0xFED8
CONST uint8 ppsp_serv_char_fed8_uuid[ATT_BT_UUID_SIZE] =
{
...
static uint8
__ppsp_serv_char_fed5_prop = GATT_PROP_WRITE;

// Characteristic 2 Value
static uint8
__ppsp_serv_char_fed5_para[PPSP_SERV_CFGS_CHAR_FED5_DLEN];
...
static uint8
__ppsp_serv_char_fed7_prop = GATT_PROP_WRITE_NO_RSP;

// Characteristic 4 Value
static uint8
__ppsp_serv_char_fed7_para[PPSP_SERV_CFGS_CHAR_FED7_DLEN];
...
static uint8
__ppsp_serv_char_fed8_prop = GATT_PROP_READ | GATT_PROP_NOTIFY;   // to change to write only, HZF

// Characteristic 5 Value
static uint8
__ppsp_serv_char_fed8_para[PPSP_SERV_CFGS_CHAR_FED8_DLEN];
static uint16
__ppsp_serv_char_fed8_para_size = 0;
// Simple Profile Characteristic 1 Configuration Each client has its own
// instantiation of the Client Characteristic Configuration. Reads of the
// Client Characteristic Configuration only shows the configuration for
// that client and writes only affect the configuration of that client.
#if 0
static gattCharCfg_t __ppsp_serv_char_fed8_cfgs[GATT_MAX_NUM_CONN];
#else
static uint16 __ppsp_serv_char_fed8_cfgs = GATT_CFG_NO_OPERATION;
...
static gattAttribute_t simpleProfileAttrTbl[] =
{
    ...
    // Characteristic 1 Declaration
    {
        ...
        GATT_PERMIT_READ,
        0,
        &__ppsp_serv_char_fed5_prop
    },
    // Characteristic Value 1
    {
        { ATT_BT_UUID_SIZE, ppsp_serv_char_fed5_uuid },
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        __ppsp_serv_char_fed5_para
    },
    ...
    // Characteristic 2 Declaration
    {
        ...
        GATT_PERMIT_READ,
        0,
        &__ppsp_serv_char_fed7_prop
    },
    // Characteristic Value 2
    {
        { ATT_BT_UUID_SIZE, ppsp_serv_char_fed7_uuid },
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        __ppsp_serv_char_fed7_para
    },
    ...
    // Characteristic 3 Declaration
    {
        ...
        GATT_PERMIT_READ,
        0,
        &__ppsp_serv_char_fed8_prop
    },
    // Characteristic Value 3
    {
        { ATT_BT_UUID_SIZE, ppsp_serv_char_fed8_uuid },
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        __ppsp_serv_char_fed8_para
    },
    // Characteristic 3 configuration
    {
        ...
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        (uint8*)(&__ppsp_serv_char_fed8_cfgs)
    },
                                     ...
                                     uint8* pValue, uint8* pLen, uint16 offset, uint8 maxLen )
{
    ...
    if ( pAttr->type.len == ATT_BT_UUID_SIZE )
    {
        ...
        case PPSP_SERV_CFGS_CHAR_FED8_UUID:
        {
            ...
            *pLen = ssiz > maxLen ? maxLen : ssiz;
            // VOID osal_memcpy( pValue, pAttr->pValue + offset, *pLen );
            osal_memcpy( pValue, pAttr->pValue, *pLen = __ppsp_serv_char_fed8_para_size );
        }
        break;
    ...
    if ( pAttr->type.len == ATT_BT_UUID_SIZE )
    {
        ...
        case PPSP_SERV_CFGS_CHAR_FED5_UUID:
        {
            ...
            else
            {
                ...
                upda_para = PPSP_SERV_CFGS_CHAR_FED5_INDX;
            }
        }
        break;
        ...
        case PPSP_SERV_CFGS_CHAR_FED7_UUID:
        {
            ...
            else
            {
                ...
                upda_para = PPSP_SERV_CFGS_CHAR_FED7_INDX;
            }
        }
        break;
        ...
        case PPSP_SERV_CFGS_CHAR_FED8_UUID:
        {
            ...
            else
            {
                ...
                upda_para = PPSP_SERV_CFGS_CHAR_FED8_INDX;
            }
        }
        break;
        ...
        case GATT_CLIENT_CHAR_CFG_UUID:
        {
            ...
            if ( pAttr->handle == simpleProfileAttrTbl[7].handle )
            {
				...
				#else
				__ppsp_serv_char_fed8_cfgs = BUILD_UINT16( pValue[0], pValue[1]);	
                LOG("CCD =%d\n",__ppsp_serv_char_fed8_cfgs);	
				#endif
            }
        }
        break;
...
static void ppsp_serv_hdlr_conn_stat( uint16 conn_hdle, uint8 chng_type )
{
    ...
    if ( conn_hdle != LOOPBACK_CONNHANDLE )
    {
                  ...
                  ( !linkDB_Up( conn_hdle ) ) ) )
        {
            ...
            GATTServApp_InitCharCfg( conn_hdle, __ppsp_serv_char_fed8_cfgs );
...
bStatus_t ppsp_serv_add_serv(uint32 serv)
{
    ...
    GATTServApp_InitCharCfg( INVALID_CONNHANDLE, __ppsp_serv_char_fed8_cfgs );
...
bStatus_t ppsp_serv_set_para( uint8 para, void* valu, uint16 leng )
{
    ...
    case PPSP_SERV_CFGS_CHAR_FED8_INDX:
    {
    	#if 0
        uint16 cfgs = GATTServApp_ReadCharCfg( 0, __ppsp_serv_char_fed8_cfgs );

        if ( GATT_CLIENT_CFG_NOTIFY == cfgs )
        {
            if ( leng <= PPSP_SERV_CFGS_CHAR_FED8_DLEN )
            {
                osal_memcpy( __ppsp_serv_char_fed8_para, valu, __ppsp_serv_char_fed8_para_size = leng );
                // send Noti/Indi if enabled
                GATTServApp_ProcessCharCfg( __ppsp_serv_char_fed8_cfgs,
                                            __ppsp_serv_char_fed8_para,
                                            FALSE,
                                            simpleProfileAttrTbl,
                                            GATT_NUM_ATTRS( simpleProfileAttrTbl ),
                                            INVALID_TASK_ID );
                /*  send_notity_indication( __ppsp_serv_char_fed8_cfgs,
                                    __ppsp_serv_char_fed8_para,
                                    FALSE,
                                    simpleProfileAttrTbl,
                                    GATT_NUM_ATTRS( simpleProfileAttrTbl ),
                                    INVALID_TASK_ID,__ppsp_serv_char_fed8_para_size ); */
            }
            else
            {
                ret = bleInvalidRange;
		...
		#else
		if ( __ppsp_serv_char_fed8_cfgs & GATT_CLIENT_CFG_NOTIFY )
		{
			attHandleValueNoti_t Notif;
	        Notif.handle = simpleProfileAttrTbl[6].handle;
	        Notif.len = leng;
			osal_memcpy(Notif.value, (uint8 *)valu, leng);
	        uint8 ret = GATT_Notification( 0, &Notif, FALSE ); 
...
bStatus_t ppsp_serv_get_para( uint8 para, void* valu, uint16 leng )
{
    ...
    switch ( para )
    {
    case PPSP_SERV_CFGS_CHAR_FED5_INDX:
        VOID osal_memcpy( valu, __ppsp_serv_char_fed5_para, leng );
        break;

    case PPSP_SERV_CFGS_CHAR_FED7_INDX:
        VOID osal_memcpy( valu, __ppsp_serv_char_fed7_para, leng );
        break;
