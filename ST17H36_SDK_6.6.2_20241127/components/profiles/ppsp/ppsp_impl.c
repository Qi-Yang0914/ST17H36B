...
#define PPSP_OTA_BLE_DEFAULT (2400)


static uint16_t ppsp_ota_ble_int = PPSP_OTA_BLE_DEFAULT;
static uint16_t ppsp_ota_ble_int_true = PPSP_OTA_BLE_DEFAULT;
...
#define PPSP_IMPL_CFGS_PROG_ADDR_BGNS   (0x2E000)   // program data bgn address of flash
#define PPSP_IMPL_CFGS_PROG_ADDR_ENDS   (0x40000)   // program data end address of flash
...
#define PPSP_IMPL_CFGS_PROG_TYPE_FW     (0)
#define PPSP_IMPL_CFGS_PROG_TYPE_RS     (1)

#define PPSP_IMPL_CFGS_PROG_VERS_MAJR   (SDK_VER_MAJOR)
#define PPSP_IMPL_CFGS_PROG_VERS_MINR   (SDK_VER_MINOR)
#define PPSP_IMPL_CFGS_PROG_VERS_REVI   (SDK_VER_REVISION)
...
#define PPSP_IMPL_CFGS_PROG_PIDS_SIZE   (2)
#define PPSP_IMPL_CFGS_PROG_RSRV_SIZE   (2)

#define PPSP_IMPL_CFGS_PROG_BUFF_SIZE   (200)   // 4BYTE ALLIGNED
...
ppsp_impl_psh_prog_data(uint32 addr, void* valu, uint32 size)
{
    ...
    if ( 0 == __ppsp_impl_clit_hdlr->ppsp_impl_prog_writ_hdlr )
    {
        ...
        // flash address range check
        if ( (PPSP_IMPL_CFGS_PROG_ADDR_BGNS >  addr || PPSP_IMPL_CFGS_PROG_ADDR_ENDS <= addr) && (__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_FW))
        {
            return ( FAILURE );
        }
...
static uint16 __ATTR_FUNC_RAM__(ppsp_impl_cal_crc16_CCITT_FALSE)(uint16 crci, uint8* data, uint32 coun);
static uint16
...
ppsp_impl_ack_serv_msgs_vers(/* uint8 encr,  */uint8 type)
{
    ...
    __ppsp_impl_upda_buff[1] = SDK_VER_PID&0xFF;
	__ppsp_impl_upda_buff[2] = (SDK_VER_PID>>8)&0xFF;
    __ppsp_impl_upda_buff[3] = PPSP_IMPL_CFGS_PROG_VERS_MAJR; // revision
    __ppsp_impl_upda_buff[4] = PPSP_IMPL_CFGS_PROG_VERS_MINR; // minor
    __ppsp_impl_upda_buff[5] = PPSP_IMPL_CFGS_PROG_VERS_REVI; // major
    __ppsp_impl_upda_buff[6] = PPSP_IMPL_CFGS_PROG_VERS_RSRV; // reserved
    __ppsp_impl_upda_buff[7] = SDK_VER_RES&0xFF;
	__ppsp_impl_upda_buff[8] = (SDK_VER_RES>>8)&0xFF;
    __ppsp_impl_upda_frsz    = 9;
    logs_inf("trgt bins type:%x,vers:%d.%d.%d,rsrv:%x",
             // type       // major      // minor      // revision   // reserved
             __ppsp_impl_upda_buff[0],
             __ppsp_impl_upda_buff[3], __ppsp_impl_upda_buff[4], __ppsp_impl_upda_buff[5],
             __ppsp_impl_upda_buff[6]);
    uint8* msgs_data = __ppsp_impl_upda_buff;
    uint16 msgs_size = __ppsp_impl_upda_frsz;
    #if (1 == PPSP_IMPL_CFGS_MSGS_CRYP_ENAB)
    uint8 encr;
...
static bool ppsp_impl_ota_version_verify(uint32 version)
{
	/*ota version verify ,user to do*/
	return TRUE;
}
static void
ppsp_impl_prc_serv_msgs_upda(uint8* msgs)
{
    ...
    __ppsp_impl_upda_type = (uint8)logv;
    
    logs_inf("    type:%d ",       (uint8)logv);
	/* copy prog pids */
    osal_memcpy((void*)&__ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_pids, msgs_data+msgs_size, PPSP_IMPL_CFGS_PROG_PIDS_SIZE);
    msgs_size += PPSP_IMPL_CFGS_PROG_PIDS_SIZE;
    osal_memcpy(&logv, msgs_data+msgs_size-PPSP_IMPL_CFGS_PROG_PIDS_SIZE, PPSP_IMPL_CFGS_PROG_PIDS_SIZE);
    logs_inf("    pids:%04x ",    (uint16)logv);
    /* copy prog vers */
	...
	/* copy rsrv content */
    osal_memcpy((void*)&__ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_rsrv, msgs_data+msgs_size, PPSP_IMPL_CFGS_PROG_RSRV_SIZE);
    msgs_size += PPSP_IMPL_CFGS_PROG_RSRV_SIZE;
    osal_memcpy(&logv, msgs_data+msgs_size-PPSP_IMPL_CFGS_PROG_RSRV_SIZE, PPSP_IMPL_CFGS_PROG_RSRV_SIZE);
    logs_inf("    rsrv:%04x ",    (uint16)logv);
    /* copy prog size */
	...
	bool ota_permit_flag;
	if(__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_FW)
	{
		if( SDK_VER_PID != __ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_pids )
		{
			logs_err("!! FIRMWARE PID %x MISMATCH \r\n", __ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_pids);
	        return;
		}
		...
		ota_permit_flag = ppsp_impl_ota_version_verify(__ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_vers);
	}
	else
	{
		ota_permit_flag = TRUE;
	}
    ...
    {
        if(__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_RS)
            hal_flash_erase_sector(PPSP_IMPL_CFGS_PROG_ADDR_BASE + __ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_vers + (itr0<<12));
        else if(__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_FW)
    ...
    ppsp_impl_ack_serv_msgs_upda(/* encr,  */ota_permit_flag, 0, PPSP_IMPL_CFGS_PACK_FRAM_NUMB);
}
...
//    extern uint8 linkDB_NumActive(void);
    if ((__SLB_OTA_STATE_ATCIVE == g_slb_ota_state) && (conn_param[0].active == FALSE))
    {
    ...
    if (__SLB_OTA_STATE_ATCIVE == g_slb_ota_state)
    {
        ...
        {
            __ppsp_impl_clit_hdlr->ppsp_impl_appl_start_tmo_hdlr((uint16)(ppsp_ota_ble_int_true * 1.25));
        }
    }
    else
    {
...
ppsp_impl_prc_serv_msgs_pack(uint8* msgs)
{
    ...
    else
    {
        ...
        uint8* msgs_data = plds;
        uint16 msgs_size = frsz;
        ppsp_impl_psh_prog_data((__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_FW)?
        (PPSP_IMPL_CFGS_PROG_ADDR_BGNS + __ppsp_impl_upda_offs):\
        (PPSP_IMPL_CFGS_PROG_ADDR_BASE + __ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_vers + __ppsp_impl_upda_offs),
        msgs_data,
        msgs_size);
...
ppsp_impl_prc_serv_msgs_comp(uint8* msgs)
{
    ...
    if(__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_RS)
        addr = PPSP_IMPL_CFGS_PROG_ADDR_BASE+__ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_vers;
    uint32 crcs = 0xffff;
    ...
    {
    	uint32 tag_ota = 0x4641544f;
        if(__ppsp_impl_upda_type == PPSP_IMPL_CFGS_PROG_TYPE_FW)
        {
            ppsp_impl_psh_prog_data(PPSP_IMPL_CFGS_PROG_ADDR_BGNS, (uint8*)(&tag_ota), 4); // tag:"OTAF"
            #if (FLASH_PROTECT_FEATURE == 1)
			hal_flash_disable_lock(SLB_OTA);
        	#endif
