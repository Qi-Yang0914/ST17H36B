...
{  
	uint8_t flag[4]={0xff,0xff,0xff,0xff};
  if(spi_ptr->spi_index == SPI0)
  {
	...
	if((flag[0]!=0xFF) && (flag[0]!=0xFF) && (flag[0]!=0xFF) && (flag[0]!=0xFF))
	{
		gpio_fmux_set(sck_pin, spimap[flag[0]][1]);
		gpio_fmux_set(ssn_pin, spimap[flag[1]][1]);
		gpio_fmux_set(tx_pin, spimap[flag[2]][1]);
		gpio_fmux_set(rx_pin, spimap[flag[3]][1]);
...
int spi_bus_init(hal_spi_t* spi_ptr,spi_Cfg_t cfg)
{
  ...
  ret = spi_pin_init(spi_ptr,cfg.sclk_pin,cfg.ssn_pin,cfg.MOSI,cfg.MISO);
